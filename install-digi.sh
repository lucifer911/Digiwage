#!/bin/bash
#Version
#Info: Installs Digiwage daemon, Masternode based on privkey.
#Digiwage
...
noflags() {
echo "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄"
echo "Usage: install-digi"
echo "Example: install-digi"
echo "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄"
exit 1
}
...
message "Creating 6GB temporary swap file...this may take a few minutes..."
cd /var
sudo touch swap.img
...
sudo mv /root/digiwage-1.2.0/bin/digiwage-cli /usr/bin/
sudo mv /root/digiwage-1.2.0/bin/digiwaged /usr/bin/
sudo mv /root/digiwage-1.2.0/bin/digiwage-tx /usr/bin/
...
printf "%s\n" "rpcuser=$rpcuser" "rpcpassword=$rpcpass" "rpcallowip=127.0.0.1" "rpcport=13331" "listen=1" "server=1" "daemon=1" "maxconnections=256" "bind=$mnip" "externalip=$mnip:46003" "masternode=1" "masternodeprivkey=$MNPRIVKEY" > $CONFILE
...
success() {
digiwaged
message "SUCCESS! Your Digiwage has started. Masternode.conf setting below..."
message "MN $mnip:46003 $MNPRIVKEY TXHASH INDEX"
exit 0
}
